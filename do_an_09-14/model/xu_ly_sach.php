...
class xu_ly_sach extends data{
    ...
    function so_luong_sach(){
        $string_sql = "SELECT count(*) as so_luong_sach FROM bs_sach";
        $this->Set($string_sql);
        $this->execute();
        $result = $this->loadRow();
        return $result;
    ...
    function update_sach($thong_tin, $id_sach){
        $build_query = '';

        $bien_dem = 0;
        foreach($thong_tin as $input_name => $item_input){
            if($input_name != 'btn_cap_nhat_sach'){
                if($bien_dem == 0){
                    $build_query .= $input_name . " = '" . $item_input . "' ";
                }
                else{
                    $build_query .= ',' . $input_name . " = '" . $item_input . "' ";
                }
        ...
        $string_sql = "UPDATE bs_sach SET " . $build_query . " WHERE id = $id_sach ";
        //echo $string_sql;exit;
